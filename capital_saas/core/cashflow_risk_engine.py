...
def build_risk_signals(data: dict, metrics: dict) -> list[dict]:
    ...
    if (data.get("dso_yoy") or 0) > 20: add("orange", "应收回款周期明显拉长", "应收账款周转天数同比增加超过20%。")
    if (data.get("dio_yoy") or 0) > 30: add("orange", "库存周转恶化", "存货周转天数同比增加超过30%。")
    if data.get("dpo_yoy") is not None and data["dpo_yoy"] < -10: add("yellow", "付款账期缩短", "应付账款周转天数下降，供应商资金占用空间收窄。")
