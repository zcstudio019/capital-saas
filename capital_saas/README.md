...
后台进入 `/admin/bank-products`，点击“导入产品库”。系统同时支持 `.csv`、`.xlsx` 和 `.md`。

### 导入 Markdown 银行产品库

支持 `## 1. 【产品编码】 产品名称` 标题，及其后的 `机构`、`原章节`、`原分组` 和 `字段/内容` 二列表格。字段会自动映射，金额、利率和期限会尽量结构化，同时保留原文和未识别字段。重新导入优先按产品编码更新，无编码时按机构与产品名称更新；导入后可一键停用模拟产品。

额度、利率、期限仅作匹配参考，实际以银行审批为准。
